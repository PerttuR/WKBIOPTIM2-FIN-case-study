...
faz_sim_sample_old<-function(sampOpt, df1o){ #browser()
# Nuno Prista 2017
	...
	#browser()
												...
												#ATTT!!!!		
													...
													# analysis of frequency distributions (smooth)															
														...
														modes_sampled_individuals_smooth <- localMaxima2(as.numeric(freq_dist_sampled_indivs_smooth))
													...
													# output
														data.frame(var=variable, stages = sampling_options$stages, n_stage1 = sampling_options$stage1_samp_size, strata_var = sampling_options$strata_var, n = n_indiv, estim_weight = estim_weight_sample, NAs_x = NAs_x, mean=mean_x, se = stand_err_mean_x, cv=round(stand_err_mean_x/mean_x*100,1), min = min_x, median = median_x, max = max_x, n_class_sampled=n_class_sampled_x,
														...
														#n_modes_original = length(ls_auto_modes_sample[[variable]][["original_modes"]]), n_modes_original_smooth = length(ls_auto_modes_sample[[variable]][["smooth_modes"]]), n_modes = n_modes, n_modes_smooth = n_modes_smooth, modes_correct = modes_correct, n_modes_correct = n_modes_correct, modes_correct_smooth = modes_correct_smooth, n_modes_correct_smooth = n_modes_correct_smooth, 
...
# Nuno Prista 2017
	...
	#browser()
													...
													# analysis of frequency distributions (original)	
														modes_sampled_individuals_after_threshold<-modes_pop_estimate[modes_pop_estimate %in% which(freq_dist_pop_estimate_no_NAs> (ls_auto_modes_sample[[variable]]$min_proportion_to_accept_mode * sum(freq_dist_pop_estimate_no_NAs)))]	
														n_modes<-length(modes_sampled_individuals_after_threshold)
														# compares modes of individuals sampled with the original modes
															modes_correct<-identical(modes_sampled_individuals_after_threshold,ls_auto_modes_sample[[variable]][["original_modes"]])
															n_modes_correct<-sum(modes_sampled_individuals_after_threshold %in% ls_auto_modes_sample[[variable]][["original_modes"]])
													...
													# analysis of frequency distributions (smooth)
														aux1<-as.numeric(names(freq_dist_pop_estimate_no_NAs))
														aux<-factor(aux1-aux1%%ls_auto_modes_sample[[variable]][["smooth_class_span"]], levels=ls_auto_modes_sample[[variable]][["smooth_breaks"]])
														freq_dist_sampled_indivs_smooth <- tapply(freq_dist_pop_estimate_no_NAs, aux, sum)
														freq_dist_sampled_indivs_smooth[is.na(freq_dist_sampled_indivs_smooth)]<-0
														modes_sampled_individuals_smooth <- localMaxima2(as.numeric(freq_dist_sampled_indivs_smooth))
														modes_sampled_individuals_smooth_after_threshold<-modes_sampled_individuals_smooth[modes_sampled_individuals_smooth %in% which(freq_dist_sampled_indivs_smooth> (ls_auto_modes_sample[[variable]]$min_proportion_to_accept_mode * sum(freq_dist_sampled_indivs_smooth)))]	
														n_modes_smooth = length(modes_sampled_individuals_smooth_after_threshold)
														# compares smoothed modes of individuals sampled with the original smoothed modes														
													...
													# tests on distributions
														# CHECK - comparison of means [true value if in confidence interval] [ based on # from https://newonlinecourses.science.psu.edu/stat506/node/27/ ]
																	# ATT: must be CHECK - when n_strata=1 formula yields NaN
															#browser()
															#Satterwaithe approximation for the degrees of freedom
															#browser()
																N_strata2<-N_strata[N_strata>0][names(n_strata[n_strata>1])]
																n_strata2<-n_strata[n_strata>1]
															if(variable!=sampling_options$strata_var)
																{
																sd_strata2<-sd_strata[n_strata>1]
																} else {
																		sd_strata2<-sd_strata[names(n_strata[n_strata>1])]
																		}
															dof=sum((N_strata2*(N_strata2-n_strata2)/n_strata2)*(sd_strata2^2))^2/sum((((N_strata2*(N_strata2-n_strata2)/n_strata2)*(sd_strata2^2))^2)/(n_strata2-1))
															t_value<-qt(c(.025,.975),dof)
																
															conf_int <-	mean_pop_estimate+t_value *	stand_err_mean_pop_estimate
															ttest_logic <- in_interval(mean(df1[,variable], na.rm=T), conf_int) 
															ttest_prob <- pt( ttest_value <- (mean_pop_estimate-mean(df1[,variable], na.rm=T)) / sqrt(var_mean_pop_estimate), dof)
															if(variable==sampling_options$strata_var)
																{
																ttest_logic = TRUE
																ttest_prob = 1
														...
														# ks test
														ks_prob <- ks.test(as.numeric(rep(names(freq_dist_pop_estimate_no_NAs), round(freq_dist_pop_estimate_no_NAs))), df1[,variable])$p.value
														kstest_logic <- ks_prob>=0.05
		
														# L1, L2, L3, L2a
														realized_probs <- prop.table(freq_dist_pop_estimate_no_NAs)
														aux<-ls_auto_modes_sample[[variable]][["original_freq"]]
														expected_probs <- prop.table(aux[!is.na(names(aux))])
														L1 <- sum(abs(expected_probs-realized_probs))
														L2 <- sqrt(sum((expected_probs-realized_probs)^2))
														L2a <- sum((expected_probs-realized_probs)^2)
														L3 <- max(expected_probs-realized_probs)
													...
													# output
														data.frame(var=variable, stages = sampling_options$stages, n_stage1 = sampling_options$stage1_samp_size, strata_var = sampling_options$strata_var, n = n_indiv, estim_weight = estim_weight_sample, NAs_x = NAs_x, mean=mean_pop_estimate, se = stand_err_mean_pop_estimate, cv=round(stand_err_mean_pop_estimate/mean_pop_estimate*100,1), min = min_pop_estimate, median = median_pop_estimate, max = max_pop_estimate, n_class_sampled=n_class_sampled_pop_estimate,
														n_modes = n_modes, n_modes_smooth = n_modes_smooth, modes_correct = modes_correct, n_modes_correct = n_modes_correct, modes_correct_smooth = modes_correct_smooth, n_modes_correct_smooth = n_modes_correct_smooth, 
														#n_modes_original = length(ls_auto_modes_sample[[variable]][["original_modes"]]), n_modes_original_smooth = length(ls_auto_modes_sample[[variable]][["smooth_modes"]]), n_modes = n_modes, n_modes_smooth = n_modes_smooth, modes_correct = modes_correct, n_modes_correct = n_modes_correct, modes_correct_smooth = modes_correct_smooth, n_modes_correct_smooth = n_modes_correct_smooth, 
														ttest_prob = ttest_prob, ttest_logic = ttest_logic, ks_prob = ks_prob, kstest_logic = kstest_logic, MWCV = MWCV,
														L1 = L1, L2 = L2, L3 = L3, L2a = L2a)															
